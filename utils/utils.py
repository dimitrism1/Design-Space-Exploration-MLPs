...
class utils():
                ...
                if(i == 22):
                    ...
                    if len(d) > 1:
                        ...
                        if(d[13].replace(" ","")!="-"):
                        	...
                        	LUT.append(0)
                        if(d[11].replace(" ","")!="-"):
                        	self.DSP.append(int(d[11].replace(" ","").split("(")[0]))
                        else:
                        	self.DSP.append(0)
    ...
    def gather(self,loaded_model,prec = 8,reuse = 1,DSP = True,accum_bits=16,accum_int_bits=6,suppress = False):
        ...
        return DSP_pred,LUT_pred,FF_pred
    ...
    def write_to_files(self,Neural_name,reuse,precision_min=2,precision_max=8,destination = 'results'):
        for precision in range(precision_min,precision_max + 1,1):
            ...
            for i in range(model_num):
                ...
                DSP,FF,LUT = self.get_resources(resource_path + "/model_" + str(i),reuse)
                LUT_pred,FF_pred,DSP_pred = self.gather(precision,model,reuse,True,16,6)
